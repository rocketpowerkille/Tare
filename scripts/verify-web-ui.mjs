...
import { checkWorkspace, checkEulerDiscovery, checkWalletNavigation } from './web-workspace-checks.mjs';
...
import { checkV2Accounting } from './web-v2-accounting-checks.mjs';
...
try {
  ...
  assert.deepEqual(await page.locator('.bazantic-steps li').first().locator('code').allInnerTexts(), ['npm i -g @bazantic/cli@latest', 'baz login']);
  ...
  await page.getByRole('button', { name: 'Copy login command', exact: true }).getByRole('status').filter({ hasText: 'Copied' }).waitFor();
  assert.equal(await page.locator('.bazantic-guide input[type=checkbox]').count(), 0);
  const example = page.locator('.access-token-example');
  assert.match(await example.innerText(), /not a working access code/);
  ...
  assert.match(tokenCommand, /body\?\.accessToken/);
  assert.doesNotMatch(tokenCommand, /clipboard|clip\.exe/);
  assert.equal(await page.getByText('Mark steps as you finish them.', { exact: false }).count(), 0);
  ...
  await checkWalletNavigation({ page, origin, fixture, capabilities });
  ...
  await checkV2Accounting({ page, origin, fixture, capabilities, fits });
