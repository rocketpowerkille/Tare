...
| Bazantic | Gateway `zvnss2njirhqjllnbfsv3sneca` is live against the Render API in Base Sepolia sandbox mode. Its MCP server exposes six tools, including wallet discovery, compact analysis and short-lived session issuance. A paid sandbox request completed the 402 quote, Base Sepolia test-USDC authorization, upstream retry and 15-minute session issuance. The published `DeFi Vault Backing Evidence Evaluator` and `tare-graph-accounting-assurance` Recipes are also live. The latter binds the direct Graph Studio gateway with Tare's compact accounting tool and returned `matched` after 56/56 accounting checks. | Preserve the paid-session receipt and two-service Recipe output. Customer calls use the public gateway through `baz curl`; Bazantic Playground intentionally lists only gateways owned by the current account. Mainnet payment access is not enabled. |
...
- Bazantic: the hosted sandbox gateway now exposes a six-tool MCP surface. The
  ...
  `tare_start_bazantic_sandbox_session`. Base Sepolia sandbox settlement and
  short-lived session issuance completed through the live gateway.
