...
| Bazantic | Gateway `zvnss2njirhqjllnbfsv3sneca` is live against the Render API in Base Sepolia sandbox mode. Its MCP server exposes six tools, including wallet discovery, compact analysis and short-lived session issuance. The published `DeFi Vault Backing Evidence Evaluator` Recipe completed retained and live Base Sepolia runs without payment. The direct Graph Studio gateway `hgtvwubvqvci5fddvkmksjtkdu` is active, and the draft `tare-graph-accounting-assurance` Recipe binds its Graph tool with Tare's compact accounting tool. Live tests reached both services and exposed one final issue: the deployed compact accounting response was 36,225 bytes, above Bazantic's 32 KiB result limit. The local response now summarizes all 56 checks and the full acceptance suite passes. | Commit and redeploy the compact-response fix, rerun the two-service Recipe, then publish it after a successful result. For paid sandbox acceptance, fund the receiving wallet with Base Sepolia USDC from Circle's faucet and confirm it under Bazantic Settings, Balances, Advanced. |
| Graph composition | Token API plus Studio accounting composition, same-block RPC checks, CLI/API/MCP/UI exposure and retained replay are implemented. Hosted acceptance passed on 2026-09-12: the Token API share balance matched RPC and the Studio accounting result matched all 56 RPC reads with no findings. | Preserve the hosted acceptance output and record the concise submission demo. Do not present it as historical share-ledger acceptance. |
...
- The Graph product composition: the hosted Render API combined the live Token API
  balance and Studio accounting subgraph, checked both against RPC at the indexed
  block, and passed with **56 accounting reads** and no findings.
...
- Bazantic: the hosted sandbox gateway now exposes a six-tool MCP surface. The
  ...
  The current surface includes `tare_discover_vaults` and
  `tare_start_bazantic_sandbox_session`. Base Sepolia sandbox settlement is
  enabled, but the paid-path acceptance is waiting for sandbox USDC.
