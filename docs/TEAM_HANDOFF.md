...
| Bazantic | Gateway `zvnss2njirhqjllnbfsv3sneca` is live against the Render API in Base Sepolia sandbox mode. Its MCP server exposes six tools, including wallet discovery, compact analysis and short-lived session issuance. The published `DeFi Vault Backing Evidence Evaluator` Recipe completed retained and live Base Sepolia runs without payment. The direct Graph Studio gateway `hgtvwubvqvci5fddvkmksjtkdu` is active, and the published `tare-graph-accounting-assurance` Recipe binds its Graph tool with Tare's compact accounting tool. The post-deploy acceptance passed on 2026-09-12: The Graph was healthy at block `25961875`, Tare matched 56/56 accounting checks with zero findings in `7.94s`, and Bazantic returned `matched`. | The paid sandbox lifecycle is separate: fund the receiving wallet with Base Sepolia USDC from Circle's faucet. Bazantic's dashboard currently hides the testnet balance, so confirm funding through the Base Sepolia transaction or a direct token-balance read. |
