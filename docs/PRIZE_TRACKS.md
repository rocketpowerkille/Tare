...
| Best Recipe that uses ETHGlobal sponsor APIs | Live acceptance passed, publication pending | The direct Graph Studio gateway is active and the draft `tare-graph-accounting-assurance` Recipe binds its `graph_tare_accounting_head` tool with Tare's `tare_analyze_compact` tool. On 2026-09-12 the direct Graph call reported a healthy expected deployment at block `25961875`, Tare matched all 56 accounting checks with zero findings, and Bazantic returned `matched`. Publish the reviewed Recipe before submission. |
| Agentify a New API | Live acceptance passed, publication pending | Tare is a new API with a live six-tool sandbox gateway. The two-service Recipe calls The Graph Studio directly and then calls Tare for the bounded accounting assurance result. The post-deploy test passed with both tools. Publish the reviewed Recipe and preserve the test result for submission evidence. |
