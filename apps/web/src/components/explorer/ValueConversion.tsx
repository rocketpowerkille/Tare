...
export function ValueConversion({ report, modules = [] }: { report: JsonRecord; modules?: JsonRecord[] }) {
  ...
  return <section className="value-conversion" aria-label="From shares to estimated value">
    <div className="value-steps">
      ...
      <article><span className={`value-label ${usd.amount ? '' : 'uncertain'}`}>{usd.amount ? 'Market-priced' : 'Unavailable'}</span><h4>Estimated USD value</h4><strong>{usd.amount ? `$${usd.amount}` : 'Not priced'}</strong><small>{usd.amount ? 'Accounting quote × Chainlink reference price.' : 'No qualifying price returned for this report.'}</small></article>
    </div>
    {usd.amount && <div className="price-provenance"><span>Chainlink · {usd.quote ? `$${usd.quote} per unit` : 'Reference price'}</span><span>Price timestamp: {usd.updatedAt ?? 'Unavailable'}</span>{usd.feed && <CopyValue value={usd.feed} label="Chainlink feed address" />}</div>}
