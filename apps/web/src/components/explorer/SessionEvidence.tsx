...
export function SessionEvidence({ token, compact = false }: { token: string; compact?: boolean }) {
  ...
  return <section className={`session-evidence${compact ? ' session-evidence-compact' : ''}`} aria-label="Payment authorization evidence">
    ...
    {compact ? <p>Access expires: {displayTimestamp(claims.expiresAt) ?? 'Unavailable'}</p> : <><h2>Payment authorization received</h2>{sessionDetails}</>}
    <details><summary>Session and receipt details</summary>{compact && sessionDetails}<p>Payment amount, asset and settlement receipt are not included in this access token. Keep the original Bazantic purchase response as payment evidence.</p></details>
