...
const analyzeSchema = {
  ...
  properties: {
    ...
    vault: address,
    blockNumber,
    blockHash: { type: 'string', pattern: '^0x[0-9a-fA-F]{64}$', description: 'Optional expected position block hash for value-position. A mismatch rejects valuation.' },
    chainId: { type: 'integer', enum: [1, 8453, 42161, 84532], default: 1 },
    asset: { ...address, description: 'For value-position: allowlisted native USDC or canonical WETH on chain 1, 8453 or 42161. No testnet pricing.' },
