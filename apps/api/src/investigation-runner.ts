...
type Run = { id: string; status: 'running' | 'complete' | 'review-required' | 'unavailable'; recipe: string;
  startedAt: string; finishedAt?: string; reportDigest: string; sections?: z.infer<typeof Answer>['sections'];
  ...
  private async execute(run: Run, reference: string, question: string) {
    try {
      ...
      if (typeof output === 'string') {
        try { output = JSON.parse(output.replace(/^\s*```(?:json)?\s*/, '').replace(/\s*```\s*$/, '')); } catch { output = null; }
      }
      ...
      if (!parsed.success || !this.store.allPagesRead(reference)
        || parsed.data.sections.some((section, index) => section.title !== answerSections[index]
          || section.citations.some(id => !ids.has(id))
          || section.text.includes(reference)
          || /tare_sandbox_v1\.|Bearer\s|-----BEGIN .*PRIVATE KEY-----/i.test(section.text))) {
        run.status = 'review-required';
        run.warning = 'The Recipe did not return the required cited answer or did not retrieve all context pages. Its answer is withheld. Review the Recipe definition; your technical report is unchanged.';
      } else {
        run.status = 'complete';
        run.sections = parsed.data.sections;
