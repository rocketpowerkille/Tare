...
test('configuration stays private, OpenAPI describes strict requests, and explorer assets are served', async () => {
  ...
  await withApi(async url => {
    ...
    for (const [path, type] of [['/', 'text/html'], ['/explore', 'text/html'], ['/investigate', 'text/html'], ['/docs', 'text/html'], ['/developers', 'text/html'], ['/assets/app.js', 'text/javascript'], ['/assets/app.css', 'text/css'], ['/favicon.svg', 'image/svg+xml'], ['/tare-mark.png', 'image/png'], ['/tare-logo.png', 'image/png']]) {
