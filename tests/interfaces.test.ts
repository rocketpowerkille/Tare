...
test('configuration stays private, OpenAPI describes strict requests, and explorer assets are served', async () => {
  ...
  await withApi(async url => {
    ...
    assert.equal(discoveryTool.post.operationId, 'tare_discover_vaults');
    assert.deepEqual(discoveryTool.post.requestBody.content['application/json']?.schema.required, ['owner']);
    ...
    for (const [path, type] of [['/', 'text/html'], ['/explore', 'text/html'], ['/investigate', 'text/html'], ['/examples', 'text/html'], ['/docs', 'text/html'], ['/developers', 'text/html'], ['/assets/app.js', 'text/javascript'], ['/assets/app.css', 'text/css'], ['/favicon.svg', 'image/svg+xml'], ['/tare-mark.png', 'image/png'], ['/tare-logo.png', 'image/png']]) {
