...
test('generic ERC-4626 accounting and replay use the selected Ethereum, Base or Arbitrum chain', async t => {
  for (const chainId of [1, 8453, 42161]) {
    await t.test(String(chainId), async () => {
      ...
      await withServer((body, response) => {
        ...
        if (call.method === 'eth_chainId') return json(response, { jsonrpc: '2.0', id: call.id, result: `0x${chainId.toString(16)}` });
        ...
        const report = await resolveErc4626Position({ owner, vault, chainId: chainId as 1 | 8453 | 42161, rpcUrl });
        ...
        assert.equal(replay.chainId, chainId);
        await assert.rejects(resolveErc4626Position({ owner, vault, chainId: chainId === 1 ? 8453 : 1, rpcUrl }), /chain/i);
      });
    });
  }
});
