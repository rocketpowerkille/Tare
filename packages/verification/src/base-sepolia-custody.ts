...
import { BaseSepoliaCustodyCaptureSchema, BaseSepoliaCustodyDeploymentSchema } from './base-sepolia-custody-capture.js';
import type { BaseSepoliaCustodyCapture } from './base-sepolia-custody-capture.js';

export const BASE_SEPOLIA_CHAIN_ID = 84532;
const MAX_UINT256 = 2n ** 256n - 1n;

function deploymentFindings(capture: ReturnType<typeof BaseSepoliaCustodyCaptureSchema.parse>, index: number) {
...
function accountingFindings(
  ...
  capture: ReturnType<typeof BaseSepoliaCustodyCaptureSchema.parse>,
) {
  const findings: string[] = [];
...
async function evaluateBaseSepoliaCustody(input: unknown, sourceMode: 'live-rpc' | 'recorded-rpc') {
  const capture = BaseSepoliaCustodyCaptureSchema.parse(input);
  ...
  return {
    reportType: 'base-sepolia-custody' as const,
    schemaVersion: 1,
    sourceMode,
    ...
    metric: terminal === null
      ...
      : { kind: 'available' as const, scope: 'base-sepolia-two-layer-control' as const,
        terminalAsset: capture.deployment.terminalAsset,
...
export function replayBaseSepoliaCustody(input: unknown) {
  return evaluateBaseSepoliaCustody(input, 'recorded-rpc');
}

const VerifyOptionsSchema = z.strictObject({
  owner: AddressSchema,
  deployment: BaseSepoliaCustodyDeploymentSchema,
...
/** Acquire the allowlisted Base Sepolia control from two differently hosted RPC endpoints. */
export async function verifyBaseSepoliaCustody(input: z.input<typeof VerifyOptionsSchema>) {
  ...
  if (providerIds[0] === providerIds[1]) {
    throw new Error('Base Sepolia custody verification requires different RPC hostnames');
  }
  const readers = urls.map(url => new PinnedRpc(url, options.timeoutMs, 20, 120000)) as [PinnedRpc, PinnedRpc];
  const capture: BaseSepoliaCustodyCapture = {
    captureVersion: 1,
    scope: 'base-sepolia-two-layer-erc4626-custody',
    ...
    witnesses: readers.map((reader, index) => ({
      ...
      codes: reader.codes,
    })) as BaseSepoliaCustodyCapture['witnesses'],
    failures: [],
  };

  try {
    await readers[0].pin(BASE_SEPOLIA_CHAIN_ID, options.blockNumber);
    await readers[1].pin(BASE_SEPOLIA_CHAIN_ID, BigInt(readers[0].block.number).toString());
  ...
  return evaluateBaseSepoliaCustody(BaseSepoliaCustodyCaptureSchema.parse(capture), 'live-rpc');
}
